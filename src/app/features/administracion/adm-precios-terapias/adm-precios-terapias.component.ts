...
@Component({
    selector: 'app-adm-precios-terapias',
    standalone: true,
    imports: [CommonModule, FormsModule],
    templateUrl: './adm-precios-terapias.component.html',
    styleUrls: ['./adm-precios-terapias.component.scss']
})
export class AdmPreciosTerapiasComponent implements OnInit {
    Math = Math;
    items: PrecioTerapia[] = [];
    filteredItems: PrecioTerapia[] = [];
    searchTerm = ''; statusFilter = 'all';
    currentPage = 1; pageSize = 10;
    openMenuId: string | null = null;
    showModal = false; editingItem: PrecioTerapia | null = null; formData: Partial<PrecioTerapia> = {};
    ...
    applyFilters(): void {
        this.filteredItems = this.items.filter(i => {
            const s = !this.searchTerm || i.tipoCliente.toLowerCase().includes(this.searchTerm.toLowerCase()) || i.specialty.toLowerCase().includes(this.searchTerm.toLowerCase());
    ...
    get paginatedItems(): PrecioTerapia[] { const s = (this.currentPage - 1) * this.pageSize; return this.filteredItems.slice(s, s + this.pageSize); }
    ...
    openEditModal(item: PrecioTerapia): void { this.editingItem = item; this.formData = { ...item }; this.showModal = true; this.openMenuId = null; }
    ...
    saveItem(): void {
        if (this.editingItem) {
            ...
            this.items.unshift({ id: `TPR${String(this.items.length + 1).padStart(3, '0')}`, tipoCliente: this.formData.tipoCliente || '', specialty: this.formData.specialty || '', precio: this.formData.precio || 0, status: this.formData.status || 'ACTIVE' });
        }
