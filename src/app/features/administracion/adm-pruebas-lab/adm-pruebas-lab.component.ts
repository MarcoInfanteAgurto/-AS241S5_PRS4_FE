...
@Component({
    selector: 'app-adm-pruebas-lab',
    standalone: true,
    imports: [CommonModule, FormsModule],
    templateUrl: './adm-pruebas-lab.component.html',
    styleUrls: ['./adm-pruebas-lab.component.scss']
})
export class AdmPruebasLabComponent implements OnInit {
    Math = Math;
    items: PruebaLab[] = [];
    filteredItems: PruebaLab[] = [];
    searchTerm = ''; statusFilter = 'all';
    currentPage = 1; pageSize = 10;
    openMenuId: string | null = null;
    showModal = false; editingItem: PruebaLab | null = null; formData: Partial<PruebaLab> = {};
    stats = { total: 0, active: 0, inactive: 0, precioPromedio: 0 };
    statusOptions = ['ACTIVE', 'INACTIVE'];
    tipoMuestraOptions = ['SANGRE', 'ORINA', 'HECES', 'ESPUTO', 'OTRO'];
    ...
    applyFilters(): void {
        this.filteredItems = this.items.filter(i => {
            const s = !this.searchTerm || i.nombre.toLowerCase().includes(this.searchTerm.toLowerCase());
    ...
    get paginatedItems(): PruebaLab[] { const s = (this.currentPage - 1) * this.pageSize; return this.filteredItems.slice(s, s + this.pageSize); }
    ...
    openEditModal(item: PruebaLab): void { this.editingItem = item; this.formData = { ...item }; this.showModal = true; this.openMenuId = null; }
    ...
    saveItem(): void {
        if (this.editingItem) {
            ...
            this.items.unshift({ id: `LAB${String(this.items.length + 1).padStart(3, '0')}`, nombre: this.formData.nombre || '', precio: this.formData.precio || 0, tipoMuestra: this.formData.tipoMuestra || 'SANGRE', tiempoResultado: this.formData.tiempoResultado || '', status: this.formData.status || 'ACTIVE' });
        }
