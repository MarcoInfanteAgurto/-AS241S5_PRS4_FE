...
export class EgresosComponent implements OnInit {
  items: Egreso[] = [];
  filteredItems: Egreso[] = [];
  paginatedItems: Egreso[] = [];
  ...
  categoryOptions = ['GASTO_CAMPAÑA', 'COMPRA', 'SERVICIO', 'OTRO'];

  showModal = false;
  editingItem: Egreso | null = null;
  ...
  get stats() {
    return {
      ...
      pagados: this.items.filter(i => i.status === 'PAID').length,
  ...
  openAddModal() {
    ...
    this.form = { transactionType: 'EGRESO', currency: 'PEN', status: 'PENDING' };
    this.showModal = true;
  }

  openEditModal(item: Egreso) {
  ...
  saveItem() {
    if (this.editingItem) {
      ...
      if (idx !== -1) this.items[idx] = { ...this.editingItem, ...this.form } as Egreso;
    } else {
      this.items.push({
