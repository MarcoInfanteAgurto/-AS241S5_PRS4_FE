...
export class RepLaboratorioComponent implements OnInit {
  items: RepLaboratorio[] = [];
  filteredItems: RepLaboratorio[] = [];
  paginatedItems: RepLaboratorio[] = [];
  ...
  get stats() {
    return {
      ...
      consignados: this.items.filter(i => i.estado === 'CONSIGNADO').length,
      donados: this.items.filter(i => i.estado === 'DONADO').length,
  ...
  verDetalle(item: RepLaboratorio) { this.openMenuId = null; }
